...
class SEResNet(nn.Module):
    def __init__(self, block = None, blocks = None, zero_init_residual = False,
                 ...
                 norm_layer=None, extras = None, fusion = False):
        super().__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
...
@registry.BACKBONES.register('SEResnet18')
def SEResnet18(cfg, pretrained=True):
    model = SEResNet(SEBasicBlock, blocks=cfg.MODEL.RESNET.BLOCKS, extras=cfg.MODEL.RESNET.EXTRAS, fusion = cfg.MODEL.RESNET.FUSION)
    if pretrained:
...
@registry.BACKBONES.register('SEResnet34')
...
@registry.BACKBONES.register('SEResnet50')
def SEResnet50(cfg, pretrained=True):
    model = SEResNet(SEBottleneck, blocks = cfg.MODEL.RESNET.BLOCKS, extras = cfg.MODEL.RESNET.EXTRAS, fusion = cfg.MODEL.RESNET.FUSION)
    if pretrained:
...
@registry.BACKBONES.register('SEResnet101')
def SEResnet101(cfg, pretrained=True):
    model = SEResNet(SEBottleneck, blocks = cfg.MODEL.RESNET.BLOCKS, extras = cfg.MODEL.RESNET.EXTRAS, fusion = cfg.MODEL.RESNET.FUSION)
    if pretrained:
...
@registry.BACKBONES.register('SEResnet152')
def SEResnet152(cfg, pretrained=True):
    model = SEResNet(SEBottleneck, blocks = cfg.MODEL.RESNET.BLOCKS, extras = cfg.MODEL.RESNET.EXTRAS, fusion = cfg.MODEL.RESNET.FUSION)
    if pretrained:
...
@registry.BACKBONES.register('SEResnet50_32x4d')
def SEResnet50_32x4d(cfg, pretrained=True):
    model = SEResNet(SEBottleneck, blocks = cfg.MODEL.RESNET.BLOCKS, extras = cfg.MODEL.RESNET.EXTRAS,
                   groups = 32, width_per_group = 4, fusion = cfg.MODEL.RESNET.FUSION)
    if pretrained:
...
@registry.BACKBONES.register('SEResnet101_32x8d')
def SEResnet101_32x8d(cfg, pretrained=True):
    model = SEResNet(SEBottleneck, blocks = cfg.MODEL.RESNET.BLOCKS, extras = cfg.MODEL.RESNET.EXTRAS,
                   groups = 32, width_per_group = 8, fusion = cfg.MODEL.RESNET.FUSION)
    if pretrained:
...
@registry.BACKBONES.register('SEwide_resnet50_2')
def SEwide_resnet50_2(cfg, pretrained=True):
    model = SEResNet(SEBottleneck, blocks = cfg.MODEL.RESNET.BLOCKS, extras = cfg.MODEL.RESNET.EXTRAS,
                   width_per_group = 64 * 2, fusion = cfg.MODEL.RESNET.FUSION)
    if pretrained:
...
@registry.BACKBONES.register('SEwide_resnet101_2')
def SEwide_resnet101_2(cfg, pretrained=True):
    model = SEResNet(SEBottleneck, blocks = cfg.MODEL.RESNET.BLOCKS, extras = cfg.MODEL.RESNET.EXTRAS,
                   width_per_group = 64 * 2, fusion = cfg.MODEL.RESNET.FUSION)
    if pretrained:
